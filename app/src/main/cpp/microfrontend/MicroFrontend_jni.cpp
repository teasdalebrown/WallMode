...
// approach.
//
// JNI entry points target io.github.rvbcrs.wallmode.MicroFrontend rather than
// HA's io.homeassistant.companion.android.microfrontend.MicroFrontend.
...
Java_io_github_rvbcrs_wallmode_MicroFrontend_nativeCreate(
...
Java_io_github_rvbcrs_wallmode_MicroFrontend_nativeDestroy(
...
Java_io_github_rvbcrs_wallmode_MicroFrontend_nativeProcessSamples(
...
JNIEXPORT void JNICALL
Java_io_github_rvbcrs_wallmode_MicroFrontend_nativeReset(
    JNIEnv* env, jclass clazz, jlong handle) {
    if (handle == 0) {
        return;
    }
